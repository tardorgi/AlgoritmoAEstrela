...
// Função para verificar se uma coordenada está na lista
...
// Ordenar lista pelo custo total (g + h)
...
// Recuperar o caminho a partir da lista fechada
...
void buscar() {
  ...
  while (tamanhoAberta > 0) {
    ...
    printf("Explorando nó: (%d, %d) - g(x): %d, h(x): %d\n", atual.coord.x,
           atual.coord.y, atual.custo, atual.heuristica);
    ...
    for (int i = 0; i < numVizinhos; i++) {
      ...
      if (melhorCaminho) {
        ...
        printf("Adicionando vizinho: (%d, %d) - g(x): %d, h(x): %d\n",
               vizinho.x, vizinho.y, novoCusto,
               distanciaManhattan(vizinho, final));
      }
    }
  }
...
void carregarMapa(const char *arquivo) {
  ...
  printf("Dimensões do mapa: %d x %d\n", linhas, colunas);
...
int main(int argc, char *argv[]) {
  ...
  if (inicio.x < 0 || inicio.x >= linhas || inicio.y < 0 ||
      inicio.y >= colunas || final.x < 0 || final.x >= linhas || final.y < 0 ||
      final.y >= colunas) {
  ...
  if (mapa[inicio.x][inicio.y] == OBSTACLE ||
    ...
    printf("Erro: Ponto inicial ou final é um obstáculo.\n");
    return 1;
  }
